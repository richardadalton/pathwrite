...
type FacadeMethodNames = {
  [K in keyof PathFacade]: PathFacade[K] extends Signal<unknown> ? never : PathFacade[K] extends (...args: never[]) => unknown ? K : never
}[keyof PathFacade];
describe("usePathContext() exposes every PathFacade method", () => {
  it("every facade method except engine plumbing, the lifecycle hook and snapshot() (exposed as a signal) is forwarded", () => {
    expectTypeOf<Exclude<FacadeMethodNames, "adoptEngine" | "ngOnDestroy" | "snapshot">>().toEqualTypeOf<FacadeContextMethod>();
  });
