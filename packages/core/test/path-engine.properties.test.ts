...
const arbGuardedPath: fc.Arbitrary<PathDefinition> = fc
  ...
  .chain((ids) =>
    fc.array(fc.boolean(), { minLength: ids.length, maxLength: ids.length }).map((guards) => ({
      id: "test",
    ...
    await fc.assert(
      fc.asyncProperty(arbPlainPath, arbActions, async (path, actions) => {
        ...
        for (const action of actions) {
          ...
          if (action === "next") await engine.next();
    ...
    await fc.assert(
      fc.asyncProperty(arbPlainPath, arbActions, async (path, actions) => {
        ...
        for (const action of actions) {
          ...
          const snap = engine.snapshot();
          if (snap) {
            expect(snap.isLastStep).toBe(snap.stepIndex === snap.stepCount - 1 && snap.nestingLevel === 0);
          }
        }
      })
    );
...
describe("PathEngine (property) — guard contracts", () => {
  it("canMoveNext returning false always blocks forward navigation", async () => {
    await fc.assert(
      fc.asyncProperty(arbGuardedPath, arbActions, async (path, actions) => {
        const engine = new PathEngine();
        await engine.start(path);
        for (const action of actions) {
          const snap = engine.snapshot();
...
describe("PathEngine (property) — shouldSkip", () => {
  it("the active step never has shouldSkip() returning true", async () => {
    await fc.assert(
      fc.asyncProperty(arbSkippablePath, arbActions, async (path, actions) => {
        const engine = new PathEngine();
        await engine.start(path);
        for (const action of actions) {
          const cur = engine.snapshot();
...
describe("PathEngine (property) — sub-path nesting", () => {
  it("nestingLevel always equals the active sub-path stack depth", async () => {
    await fc.assert(
      fc.asyncProperty(fc.integer({ min: 1, max: 4 }), async (depth) => {
        const engine = new PathEngine();
