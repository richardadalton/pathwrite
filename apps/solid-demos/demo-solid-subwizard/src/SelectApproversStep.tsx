...
export default function SelectApproversStep() {
  ...
  const selected = createMemo(() => (ctx.snapshot()?.data.approvers ?? []) as string[]);
  ...
  return (
    <Show when={ctx.snapshot()}>
      <div class="form-body">
        <p class="step-intro">
          Choose who needs to approve this document. All selected approvers must review before the workflow
          can proceed.
        </p>
        ...
        <div>
          <p
            class="section-title"
            style="text-transform: none; letter-spacing: 0; font-size: 14px; font-weight: 500; color: #374151;"
          >
            Available Approvers
          </p>
