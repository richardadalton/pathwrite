        ...
        <div class="member-list">
          @for (member of members(); track $index) {
            <div class="member-row">
              ...
              <button type="button" class="btn-remove" (click)="removeMember($index)" title="Remove member">
                ✕
              </button>
            </div>
          }
...
export class TeamSetupStepComponent {
  ...
  protected readonly attempted = computed(() => this.path.snapshot()?.hasAttemptedNext ?? false);

  protected readonly teamName = computed(() => (this.path.snapshot()?.data.teamName as string) ?? "");

  protected readonly members = computed(() => (this.path.snapshot()?.data.members ?? []) as Person[]);
  ...
  protected removeMember(index: number): void {
    this.path.setData(
      "members",
      this.members().filter((_, i) => i !== index)
    );
  }
  ...
  protected updateMemberName(index: number, value: string): void {
    this.path.setData(
      "members",
      this.members().map((m, i) => (i === index ? { ...m, name: value } : m))
    );
  }
  ...
  protected updateMemberRole(index: number, value: string): void {
    this.path.setData(
      "members",
      this.members().map((m, i) => (i === index ? { ...m, role: value } : m))
    );
  }
}
