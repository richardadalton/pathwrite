    ...
    <div class="form-body">
      ...
      @if (!guardRunning()) {
        <p class="hint">
          ...
          <code>svc.checkEligibility()</code> and the engine awaits the result before deciding whether to
          advance. While it runs, <code>snapshot().status === "validating"</code>
...
export class EligibilityStepComponent {
  ...
  protected readonly guardRunning = computed(() => this.path.snapshot()?.status === "validating");
