    ...
    <div class="form-body">
      ...
      <div class="field" [class.field--error]="errors()['shippingName']">
        ...
        <input
          ...
          autofocus
          autocomplete="name"
          (input)="path.setData('shippingName', $any($event.target).value)"
          placeholder="Jane Smith"
        />
        @if (errors()["shippingName"]; as msg) {
      ...
      <div class="field" [class.field--error]="errors()['shippingAddress']">
        ...
        <input
          ...
          (input)="path.setData('shippingAddress', $any($event.target).value)"
          placeholder="123 Main St"
        />
        @if (errors()["shippingAddress"]; as msg) {
          <span class="field-error">{{ msg }}</span>
        }
      </div>
      ...
      <div class="row">
        <div class="field" [class.field--error]="errors()['shippingCity']">
          ...
          <input
            ...
            (input)="path.setData('shippingCity', $any($event.target).value)"
            placeholder="Dublin"
          />
          @if (errors()["shippingCity"]; as msg) {
            <span class="field-error">{{ msg }}</span>
          }
        </div>
        <div class="field" [class.field--error]="errors()['shippingPostcode']">
          ...
          <input
            ...
            (input)="path.setData('shippingPostcode', $any($event.target).value)"
            placeholder="D01 AB12"
          />
          @if (errors()["shippingPostcode"]; as msg) {
            <span class="field-error">{{ msg }}</span>
          }
        </div>
      </div>
