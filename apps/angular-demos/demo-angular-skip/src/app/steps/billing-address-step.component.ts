    ...
    <div class="form-body">
      ...
      <div class="field" [class.field--error]="errors()['billingName']">
        ...
        <input
          ...
          autofocus
          autocomplete="name"
          (input)="path.setData('billingName', $any($event.target).value)"
          placeholder="Jane Smith"
        />
        @if (errors()["billingName"]; as msg) {
      ...
      <div class="field" [class.field--error]="errors()['billingAddress']">
        ...
        <input
          ...
          (input)="path.setData('billingAddress', $any($event.target).value)"
          placeholder="456 Billing Rd"
        />
        @if (errors()["billingAddress"]; as msg) {
          <span class="field-error">{{ msg }}</span>
        }
      </div>
      <div class="row">
        <div class="field" [class.field--error]="errors()['billingCity']">
          ...
          <input
            ...
            (input)="path.setData('billingCity', $any($event.target).value)"
            placeholder="Dublin"
          />
          @if (errors()["billingCity"]; as msg) {
            <span class="field-error">{{ msg }}</span>
          }
        </div>
        <div class="field" [class.field--error]="errors()['billingPostcode']">
          ...
          <input
            ...
            (input)="path.setData('billingPostcode', $any($event.target).value)"
            placeholder="D02 XY34"
          />
          @if (errors()["billingPostcode"]; as msg) {
            <span class="field-error">{{ msg }}</span>
          }
        </div>
      </div>
...
export class BillingAddressStepComponent {
  ...
  protected get data(): SubscriptionData {
