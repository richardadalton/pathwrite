    ...
    <div class="form-body">
      ...
      <div class="field" [class.field--error]="errors()['cardNumber']">
        ...
        <input
          ...
          (input)="path.setData('cardNumber', $any($event.target).value)"
          placeholder="4242 4242 4242 4242"
        />
        @if (errors()["cardNumber"]; as msg) {
      ...
      <div class="row">
        <div class="field" [class.field--error]="errors()['cardExpiry']">
          ...
          <input
            ...
            autocomplete="cc-exp"
            (input)="path.setData('cardExpiry', $any($event.target).value)"
            placeholder="MM/YY"
          />
          @if (errors()["cardExpiry"]; as msg) {
            <span class="field-error">{{ msg }}</span>
          }
        </div>
        <div class="field" [class.field--error]="errors()['cardCvc']">
          ...
          <input
            ...
            [value]="data.cardCvc ?? ''"
            autocomplete="cc-csc"
            (input)="path.setData('cardCvc', $any($event.target).value)"
            placeholder="123"
          />
          @if (errors()["cardCvc"]; as msg) {
            <span class="field-error">{{ msg }}</span>
          }
        </div>
      </div>
