...
@Component({
  ...
  styles: [
    `
      ...
      .approver-select-item input[type="checkbox"] {
        accent-color: #2563eb;
        width: 16px;
        height: 16px;
        cursor: pointer;
      }
      .approver-select-item--selected {
        border-color: #2563eb;
        background: #eff6ff;
      }
      .approver-avatar {
      ...
      <div>
        ...
        <div class="approver-select-list">
          @for (approver of approvers; track approver.id) {
            <label
              class="approver-select-item"
              [class.approver-select-item--selected]="isSelected(approver.id)"
            >
...
export class SelectApproversStepComponent {
  ...
  protected toggle(id: string): void {
    const updated = this.isSelected(id) ? this.selected.filter((a) => a !== id) : [...this.selected, id];
