...
<template>
  <main class="page">
    <div class="page-header">
      <h1>Address Form</h1>
      <p class="subtitle">
        Demonstrates <code>StepChoice</code> — the address step automatically shows the right form based on
        ...
        <div class="summary-section">
          ...
          <template v-if="completedData.country === 'US'">
            <div class="summary-row">
              ...
              <span
                >{{ completedData.streetAddress
                }}{{ completedData.aptUnit ? `, ${completedData.aptUnit}` : "" }}</span
              >
            </div>
            <div class="summary-row">
              <span class="summary-key">City</span><span>{{ completedData.city }}</span>
            </div>
            <div class="summary-row">
              <span class="summary-key">State</span><span>{{ completedData.state }}</span>
            </div>
            <div class="summary-row">
              <span class="summary-key">ZIP</span><span>{{ completedData.zipCode }}</span>
            </div>
            <div class="summary-row"><span class="summary-key">Country</span><span>United States</span></div>
          </template>
          <template v-else>
            <div class="summary-row">
              <span class="summary-key">Line 1</span><span>{{ completedData.addressLine1 }}</span>
            </div>
            <div v-if="completedData.addressLine2" class="summary-row">
              <span class="summary-key">Line 2</span><span>{{ completedData.addressLine2 }}</span>
            </div>
            <div class="summary-row">
              <span class="summary-key">Town</span><span>{{ completedData.town }}</span>
            </div>
            <div class="summary-row">
              <span class="summary-key">County</span><span>{{ completedData.county }}</span>
            </div>
            <div v-if="completedData.eircode" class="summary-row">
              <span class="summary-key">Eircode</span><span>{{ completedData.eircode }}</span>
            </div>
