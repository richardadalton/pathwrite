...
<template>
  <main class="page">
    <div class="page-header">
      ...
      <p class="subtitle">
        Nested PathShell demo — an independent Employee Details path runs inside Step 2. Data syncs to the
        parent via <code>@event</code>; the parent's <code>fieldErrors</code> guard validates the inner data.
      </p>
    </div>
      ...
      <div class="summary">
        <div class="summary-section">
          ...
          <div class="summary-row">
            ...
            ><span>{{ completedDetails.firstName }} {{ completedDetails.lastName }}</span>
          </div>
          <div v-if="completedDetails.phone" class="summary-row">
            <span class="summary-key">Phone</span><span>{{ completedDetails.phone }}</span>
          </div>
        </div>
        <div class="summary-section">
          <p class="summary-section__title">Department</p>
          <div class="summary-row">
            <span class="summary-key">Department</span><span>{{ completedDetails.department }}</span>
          </div>
          <div v-if="completedDetails.manager" class="summary-row">
            <span class="summary-key">Manager</span><span>{{ completedDetails.manager }}</span>
          </div>
          <div v-if="completedDetails.office" class="summary-row">
            <span class="summary-key">Office</span><span>{{ completedDetails.office }}</span>
          </div>
        </div>
        <div class="summary-section">
          <p class="summary-section__title">Equipment &amp; Roles</p>
          <div class="summary-row">
            <span class="summary-key">Laptop</span><span>{{ laptopLabel }}</span>
          </div>
          <div class="summary-row">
            <span class="summary-key">Phone</span><span>{{ yesNo(completedDetails.needsPhone) }}</span>
          </div>
          <div class="summary-row">
            <span class="summary-key">Job Title</span><span>{{ completedDetails.jobTitle }}</span>
          </div>
          <div class="summary-row">
