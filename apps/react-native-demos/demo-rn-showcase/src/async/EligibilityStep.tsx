  ...
  return (
    <View style={styles.body}>
      <Text style={styles.intro}>
        Tapping <Text style={styles.bold}>Next</Text> runs an async eligibility check (~900ms). Watch the
      ...
      {blockingError ? <Text style={styles.blockingError}>{blockingError}</Text> : null}

      {!guardRunning ? (
        <View style={styles.hint}>
          <Text style={styles.hintText}>
            <Text style={styles.hintBold}>What's happening: </Text>
            <Text style={styles.code}>canMoveNext</Text> is async — the engine awaits the result before
            deciding whether to advance. While it runs,{" "}
            <Text style={styles.code}>snapshot.status === "validating"</Text> and the shell shows a loading
            indicator. If blocked, <Text style={styles.code}>snapshot.blockingError</Text> is set and we
            render it here (since <Text style={styles.code}>validationDisplay="inline"</Text> suppresses the
