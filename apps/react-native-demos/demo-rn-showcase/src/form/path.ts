  ...
  steps: [
    {
      ...
      fieldErrors: ({ data }) => ({
        ...
        message:
          (data.message?.trim()?.length ?? 0) < 10 ? "Message must be at least 10 characters." : undefined,
      }),
      fieldWarnings: ({ data }) => ({
        email:
          data.email?.includes("@gmail.") || data.email?.includes("@yahoo.")
            ? "Note: for business inquiries, a work email is preferred."
            : undefined,
        message:
          (data.message?.trim()?.length ?? 0) > 0 && (data.message?.trim()?.length ?? 0) < 50
