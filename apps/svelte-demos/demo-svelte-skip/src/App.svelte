<script lang="ts">
  ...
  function handleComplete(data: PathData) {
    completedData = data as SubscriptionData;
    isCompleted = true;
  }
...
<main class="page">
  <div class="page-header">
    ...
    <p class="subtitle">
      Conditional steps demo — <code>shouldSkip</code> hides payment and billing steps for the Free plan.
    </p>
  </div>
  ...
  {#if isCompleted && completedData}
    <section class="result-panel success-panel">
      ...
      <p>
        You're signed up for the <strong>{PLAN_LABELS[completedData.plan] ?? completedData.plan}</strong> plan.
      </p>
      <div class="summary">
        <div class="summary-section">
          <p class="summary-section__title">Plan</p>
          <div class="summary-row">
            <span class="summary-key">Plan</span><span>{PLAN_LABELS[completedData.plan]}</span>
          </div>
        </div>
        <div class="summary-section">
          <p class="summary-section__title">Shipping</p>
          <div class="summary-row">
            <span class="summary-key">Name</span><span>{completedData.shippingName}</span>
          </div>
          <div class="summary-row">
            <span class="summary-key">Address</span><span
              >{completedData.shippingAddress}, {completedData.shippingCity}
              {completedData.shippingPostcode}</span
            >
          </div>
        </div>
        {#if completedData.plan === "paid"}
          <div class="summary-section">
            ...
            <div class="summary-row">
              <span class="summary-key">Card</span><span>•••• {completedData.cardNumber.slice(-4)}</span>
            </div>
          </div>
        {/if}
      </div>
