...
{#if ctx.snapshot}
  <div class="form-body">
    ...
    <!-- Team name -->
    <div class="field" class:field--error={errors.teamName}>
      <label for="team-name">Team Name <span class="required">*</span></label>
      <input
        id="team-name"
        type="text"
        placeholder="e.g. Platform Engineering"
        value={ctx.snapshot.data.teamName ?? ""}
        oninput={(e) => ctx.setData("teamName", e.currentTarget.value)}
      />
      {#if errors.teamName}<p class="field-error">{errors.teamName}</p>{/if}
    </div>
    ...
    <div>
            ...
            <div class="member-row">
              ...
              <div class="member-fields">
                <input
                  ...
                  class:input--error={errors.members && !member.name.trim()}
                />
                <input
                  type="text"
      ...
      {#if errors.members}<p class="field-error">{errors.members}</p>{/if}
    </div>
  </div>
{/if}
