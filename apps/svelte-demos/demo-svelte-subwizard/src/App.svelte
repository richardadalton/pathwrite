<script lang="ts">
  ...
  function handleComplete(data: PathData) {
    completedData = data as DocumentData;
    isCompleted = true;
  }
  ...
  function getApprovers(d: DocumentData) {
...
<main class="page">
  ...
  {#if isCompleted && completedData}
    ...
    <section
      class="result-panel"
      class:success-panel={st === "approved"}
      class:reject-panel={st !== "approved" && st !== "mixed"}
      class:cancel-panel={st === "mixed"}
    >
      <div class="result-icon">{st === "approved" ? "✅" : "❌"}</div>
      <h2>{st === "approved" ? "Document Approved!" : "Document Rejected"}</h2>
      <p>
        {st === "approved"
          ? `All approvers signed off on "${completedData.title}".`
          : `One or more approvers rejected "${completedData.title}".`}
      </p>
      <div class="summary">
        ...
        <div class="summary-section">
          ...
          {#each getApprovers(completedData) as a}
            <div class="summary-row">
              ...
              <span
                class:text-approved={getResults(completedData)[a.id]?.decision === "approved"}
                class:text-rejected={getResults(completedData)[a.id]?.decision === "rejected"}
              >
                {getResults(completedData)[a.id]?.decision === "approved" ? "✓ Approved" : "✗ Rejected"}
                {#if getResults(completedData)[a.id]?.comment}
                  — <em>{getResults(completedData)[a.id].comment}</em>{/if}
              </span>
            </div>
          {/each}
        </div>
