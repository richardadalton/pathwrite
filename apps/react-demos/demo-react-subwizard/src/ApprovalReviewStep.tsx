...
export function ApprovalReviewStep() {
  ...
  const selectedApprovers = AVAILABLE_APPROVERS.filter((a) => (data.approvers as string[]).includes(a.id));

  const allDone = selectedApprovers.length > 0 && selectedApprovers.every((a) => !!results[a.id]?.decision);
  ...
  return (
    <div className="form-body">
        ...
        <div className="approver-review-list">
          {selectedApprovers.map((approver) => (
            <div key={approver.id} className="approver-review-item">
              ...
              {results[approver.id] ? (
                <span
                  className={`decision-badge ${results[approver.id].decision === "approved" ? "decision-badge--approved" : "decision-badge--rejected"}`}
                >
      ...
      {snap.hasAttemptedNext && errors._ && <p className="gate-message">⏳ {errors._}</p>}
      {allDone && (
        <p className="gate-message gate-message--done">
